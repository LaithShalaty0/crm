...
class Mention_model extends App_Model
{
    ...
    {   
        ...
        if($contact != []) {
            $this->db->where("POSITION('data_client_id_".$contact->userid."' IN content) > 0 or POSITION('data_contact_id_".$contact->id."' IN content) > 0 or (select count(*) from ".db_prefix()."mention_post_comments where (POSITION('data_client_id_".$contact->userid."' IN content) > 0 or POSITION('data_contact_id_".$contact->id."' IN content)) and postid = ".db_prefix()."mention_posts.id) > 0");
        }
        $this->db->order_by('datecreated', 'desc');
        $this->db->limit(5, $offset);
        return $this->db->get(db_prefix() . 'mention_posts')->result_array();
