...
class Mention extends AdminController
{
    ...
    public function load_mention($id = '', $page = 1){
        ...
        foreach ($posts as $post) {
            ...
            if (isset($post['is_contact']) && $post['is_contact'] == 1) {
                $this->load->model('clients_model');
            ...
            if (($post['creator'] == get_staff_user_id() && isset($post['is_contact']) && $post['is_contact'] == 1) || is_admin()) {
    ...
    public function get_post_data($id){
        ...
        // Check if the is_contact property exists
        $is_contact = isset($post->is_contact) ? $post->is_contact : 0;
