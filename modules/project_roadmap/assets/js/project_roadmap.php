<script>
  $(document).ready(function () {
    ...
    var project_progress_color = '#84c529'; // Replace with your desired color
  ...
  $(function () {
    var list = [];
    <?php if (isset($task_bookmarks_list) && is_array($task_bookmarks_list)) { ?>
      <?php foreach ($task_bookmarks_list as $task_bookmarks) { ?>
        list.push(<?php echo '' . $task_bookmarks['rel_id']; ?>);
      <?php } ?>
    <?php } else { ?>
      console.warn('No task bookmarks found or $task_bookmarks_list is not an array.');
    <?php } ?>
