...
class Resource_workload_model extends App_Model
{
    ...
    public function get_data_workload($data_fill, $only_data = false)
    {
        ...
        foreach ($staffsTasks as $key => $value) {
            if (!empty($value['startdate']) && strtotime($task_from_date) > strtotime($value['startdate'])) {
                $task_from_date = $value['startdate'];
            }

            if (!empty($value['duedate']) && strtotime($task_to_date) < strtotime($value['duedate'])) {
            ...
            if (isset($data_array[$value['staffid'] . '_' . $value['taskid']]) && $value['taskid'] != 0) {
                if (isset($value['total_logged_time'])) {
                    $data_array[$value['staffid'] . '_' . $value['taskid']]['taskstimers'][] = [
                        'total_logged_time' => $value['total_logged_time'],
                        'start_time' => date('Y-m-d', strtotime(to_sql_date(_dt($value['start_time'], true), true))),
                        'end_time' => date('Y-m-d', strtotime(to_sql_date(_dt($value['end_time'], true), true))),
                    ];
                }
            } else {
                $node = [];
    ...
    public function get_columns_workload($from_date, $to_date)
    {
        ...
        while (strtotime($from_date) <= strtotime($to_date)) {
            if ($visible[date('N', strtotime($from_date))] == 1) {
                array_push($columns, [
                    'data' => date('d_m_Y', strtotime($from_date)) . '_e',
                    'type' => 'numeric',
                    'readOnly' => true,
                    'numericFormat' => [
                        'pattern' => '0.00',
                ...
                array_push($columns, [
                    'data' => date('d_m_Y', strtotime($from_date)) . '_s',
                    'type' => 'numeric',
                    'readOnly' => true,
                    'numericFormat' => [
                        'pattern' => '0.00',
    ...
    public function get_columns_timeline($from_date, $to_date)
    {
        ...
        while (strtotime($from_date) <= strtotime($to_date)) {
            array_push($columns, [
                'data' => date('d_m', strtotime($from_date)),
                'type' => 'numeric',
                'readOnly' => true,
                'numericFormat' => [
                    'pattern' => '0.0',
