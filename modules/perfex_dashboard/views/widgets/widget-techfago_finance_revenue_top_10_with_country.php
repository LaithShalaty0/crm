...
$fn_get_data = function () {
  ...
  $sql = "
      SELECT 
          COUNT(id) AS orders, SUM(TBLInvoices.subtotal) AS total, TBLCountries.short_name AS country
      FROM
          " . db_prefix() . "invoices TBLInvoices
      LEFT JOIN " . db_prefix() . "countries TBLCountries ON TBLInvoices.billing_country = TBLCountries.country_id
  ...
  $sql .= "
      GROUP BY TBLInvoices.billing_country
      ORDER BY total DESC
      LIMIT 0 , 10
  ";
...
<div class="widget widget-revenue-top-10-with-country widget-<?= $widget['id'] ?>"
