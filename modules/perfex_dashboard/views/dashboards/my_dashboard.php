                    ...
                    <div class="perfexdashboard-actions-wrap perfexdashboard-actions-wrap-between">
                        <div class="perfexdashboard-actions-section-left">
                            <select name="dashboard_id" class="form-control perfexdashboard-width-select">
                                <?php foreach ($user_dashboards as $user_dashboard) { ?>
                                    <option value="<?= $user_dashboard['id'] ?>"
                                        <?= $user_dashboard['id'] == $active_dashboard_id ? 'selected' : '' ?>>
                                        <?= $user_dashboard['name'] ?></option>
                                <?php } ?>
                            </select>
                        </div>
                        <div class="perfexdashboard-actions-section-right">
                            <div class="perfexdashboard-period-select">
                                <?php
                                $tmp_req_from = $this->input->get('period_from');
                                $tmp_req_to = $this->input->get('period_to');

                                // Check if input is not null and valid date format
                                if (!empty($tmp_req_from) && DateTime::createFromFormat('Y-m-d', $tmp_req_from) !== false) {
                                    $tmp_req_from = DateTime::createFromFormat('Y-m-d', $tmp_req_from)->format('Y-m-d');
                                } else {
                                    $tmp_req_from = null;
                                }

                                if (!empty($tmp_req_to) && DateTime::createFromFormat('Y-m-d', $tmp_req_to) !== false) {
                                    $tmp_req_to = DateTime::createFromFormat('Y-m-d', $tmp_req_to)->format('Y-m-d');
                                } else {
                                    $tmp_req_to = null;
                                }

                                $tmp_req_period = '';
                                if (isset($tmp_req_from) && isset($tmp_req_to)) {
                                    $tmp_req_period = $tmp_req_from . ';' . $tmp_req_to;
                                }
                                $tmp_selected = false;
                                ?>
                                <select class="selectpicker perfexdashboard-period-dropdown" data-width="100%"
                                    data-none-selected-text="<?php echo _l('dropdown_non_selected_tex'); ?>">
                                    <option value="" <?php if ($tmp_req_period === '') {
                                        echo 'selected';
                                        $tmp_selected = true;
                                    } ?>><?php echo _l('report_sales_months_all_time'); ?>
                                    </option>
                                    <!-- Other options here -->
                                </select>
                                <!-- Date range input -->
                                    ...
                                    <div class="row">
                                        <div class="col-md-12">
                                            <div class="input-group date">
                                                <input type="text" placeholder="<?= _l('from_date') ?>"
                                                    value="<?= $tmp_req_from ?>" class="form-control datepicker"
                                                    name="period_from">
                                        ...
                                        <div class="col-md-12">
                                            <div class="input-group date">
                                                <input type="text" placeholder="<?= _l('to_date') ?>"
                                                    value="<?= $tmp_req_to ?>" class="form-control datepicker"
                                                    name="period_to">
                                                <div class="input-group-addon">
                                                    <i class="fa fa-calendar calendar-icon"></i>
                                                </div>
