    ...
    if (!function_exists('domCreateNewElement')) {
        ...
        if (!function_exists('domCreateNewElement')) {
            ...
            function domCreateNewElement($row, $data, $title = '', $text = '')
            {
                ...
                // Find the 'div' element with class 'row-options'
                $rowOptions = null;
                foreach ($doc->getElementsByTagName('div') as $div) {
                    if ($div->hasAttribute('class') && $div->getAttribute('class') === 'row-options') {
                        $rowOptions = $div;
                        break;
                    }
                }
                ...
                if ($rowOptions === null) {
                    // Handle the case where 'row-options' div is not found
                    // Optionally, create a new 'div' and add it to the document
                    $rowOptions = $doc->createElement('div');
                    $rowOptions->setAttribute('class', 'row-options');
                    $doc->appendChild($rowOptions);
                }
