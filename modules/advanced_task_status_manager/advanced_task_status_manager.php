...
function load_statuses_from_db($statuses)
{
    ...
    // Check if the URI segments are set and not empty
    if (
        isset($CI->uri->segments[2], $CI->uri->segments[3]) &&
        !empty($CI->uri->segments[2]) && !empty($CI->uri->segments[3])
    ) {

        // Check if the 'is_task_status_manager_loading' property is set
        if (isset($CI->is_task_status_manager_loading) && $CI->is_task_status_manager_loading) {
            return $statuses;
        }

        $uri_string = $CI->uri->uri_string;

        // Check if the URI string matches the specified conditions
        if ($uri_string == 'admin/tasks' || ($CI->uri->segments[2] == 'projects' && $CI->uri->segments[3] == 'view' && $CI->input->get('group') == 'project_tasks')) {

            // Get statuses that the user can't see based on staff ID
            $statusesUserCantSee = $CI->db->where('staff_id', get_staff_user_id())->get(db_prefix() . 'task_status_dont_have_staff')->result_array();
            $statusesIdsUserCantSee = array_map(function ($x) {
                return $x['task_status_id']; }, $statusesUserCantSee);
...
function load_project_statuses_from_db($statuses)
{
    ...
    // Initialize project_status_once if it's not already set
